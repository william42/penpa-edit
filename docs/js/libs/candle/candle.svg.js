...
class SVGWrapper extends WrapperBase {
    ...
    scale(horiz, vert) {
	var m = this.child.getAttribute('viewBox').split(/ /);
	m[0]/=horiz; m[1]/=vert; m[2]/=horiz; m[3]/=vert;
	this.child.setAttribute('viewBox', m.join(' '));
    }
    ...
    arcTo(x1,y1,x2,y2,r) {
	var [x0,y0] = this.cpath.slice(-2);
	console.log("TODO: actual arc");
	//this.lineTo(x1,y1); this.lineTo(x2,y2); // TODO: not this
	const islong = 0; // I think arcTo is only ever used for short arcs in penpa-edit
	const det = (y2-y1)*(x1-x0)-(x2-x1)*(y1-y0);
	const sweep = det>0?1:0; // not sure this is the right direction but there's only two so
	this.cpath.push(S_PATH_ARCTO,r,r,0,islong,sweep,x2,y2);
    }
    quadraticCurveTo(cpx, cpy, x, y){
	this.cpath.push(S_PATH_QUADRATIC, cpx, cpy, x, y);
	this.lastpath = S_PATH_QUADRATIC;
    }
    rect(x,y,w,h){
	...
	if (this.dashes.length) {
	    obj.setAttribute('stroke-dasharray', this.dashes.join(" "));
	    obj.setAttribute('stroke-dashoffset', this.lineDashOffset);
	...
	if(el.getAttribute(S_ATT_STROKE)!==this.strokeStyle){ el.setAttribute(S_ATT_STROKE, this.strokeStyle);}
	...
	if(_cache.font!==this.font){
		...
		var style = RegExp.$1, size = parseFloat(this.font.split("px")[0]), family = RegExp.$3;
